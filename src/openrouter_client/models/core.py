...
- to_plain_data: Recursively dump Pydantic models to JSON-encodable data
"""
...
def to_plain_data(value: Any) -> Any:
    """
    Recursively reduce a value to something requests' ``json=`` can encode.

    Pydantic models can appear at any depth in caller-supplied structures
    (a tool dict whose "function" is a FunctionDefinition, a list of content
    parts, a FunctionToolChoice). requests' JSON encoder cannot handle them,
    so every request-bound structure is passed through here.

    Args:
        value: The value as supplied by a caller or read off a response.
...
class ToolDefinition(BaseModel):
    ...
    type: str = Field(
        "function", description="Type of tool, currently only 'function' is supported"
    )
    function: FunctionDefinition = Field(
        ..., description="Function definition for the tool"
    )
...
class ResponseFormat(BaseModel):
    ...
    json_schema: Optional[Dict[str, Any]] = Field(
        None, description="JSON Schema for the response format"
    )
...
class CacheControl(BaseModel):
    ...
    type: str = Field(
        "ephemeral",
        description="Cache control type, currently only 'ephemeral' is supported",
    )
...
class TextContent(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        None, description="Cache control settings for this content"
    )
...
class ImageUrl(BaseModel):
    ...
    detail: Optional[str] = Field(
        "auto", description="Detail level, defaults to 'auto'"
    )
...
class ImageContentPart(BaseModel):
    ...
    image_url: ImageUrl = Field(
        ..., description="Container for image URL and detail level"
    )
...
class Prediction(BaseModel):
    ...
    type: str = Field(
        "content",
        description="Type of prediction, currently only 'content' is supported",
    )
...
class Message(BaseModel):
    ...
    function_call: Optional[Dict[str, Any]] = Field(
        None, description="Function call details if applicable"
    )
    tool_calls: Optional[List[Dict[str, Any]]] = Field(
        None, description="Tool calls details if applicable"
    )
