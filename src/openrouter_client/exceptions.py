...
- ToolExecutionError: Error while executing a model-requested tool call
- ToolCallLimitExceeded: Tool loop hit its round limit while still calling tools
"""
    ...
    def __str__(self):
        ...
        if self.details:
            ...
            for key, value in self.details.items():
                if (
                    key not in ["message", "json_error", "parse_error"]
                    ...
                    if isinstance(value, dict) and len(str(value)) > 100:
                        ...
                        detail_parts.append(
                            f"{key}: {value[:100]}... (truncated, {len(value)} chars)"
                        )
                    else:
            ...
            for key, value in self.details.items():
                if value is not None:
                    if isinstance(value, dict):
                        ...
                        if len(value) > 100:
                            lines.append(
                                f"    {value[:100]}... (truncated, {len(value)} chars)"
                            )
                        else:
        ...
        if (
            ...
            try:
                ...
                formatted_response = json.dumps(
                    parsed_response, indent=2, ensure_ascii=False
                )
...
class StreamingError(OpenRouterError):
    ...
    def __init__(self, message: str, response: Optional[Any] = None, **kwargs):
        """
        Initialize the exception.
...
class ToolExecutionError(OpenRouterError):
    """
    Exception raised when a model-requested tool call cannot be executed.

    Covers a tool name with no registered handler, arguments the model emitted
    that are not a JSON object, and a handler that raised.

    Attributes:
        message (str): Error message.
        tool_name (Optional[str]): Name of the tool the model asked for.
        tool_call_id (Optional[str]): ID of the tool call being executed.
        original_error (Optional[Exception]): Exception raised by the handler, if any.
        details (Dict[str, Any]): Additional error details.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.original_error = original_error
        super().__init__(
            message, tool_name=tool_name, tool_call_id=tool_call_id, **kwargs
        )


class ToolCallLimitExceeded(OpenRouterError):
    """
    Exception raised when a tool loop exhausts its round budget.

    Signals that the model was still requesting tool calls after ``max_rounds``
    rounds had been executed, rather than settling on an answer.

    Attributes:
        message (str): Error message.
        max_rounds (Optional[int]): The round limit that was reached.
        details (Dict[str, Any]): Additional error details.
    """
