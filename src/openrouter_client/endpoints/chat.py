...
class ChatEndpoint(BaseEndpoint):
    ...
    def _parse_streaming_response(
        self, response_iterator: Iterator[Dict[str, Any]]
            ...
            try:
                ...
                if "choices" in chunk:
                    if isinstance(chunk["choices"], dict):
                        ...
                        raise ValueError(
                            f"Unexpected choices type: {type(chunk['choices'])}"
                        )
                    ...
                    for choice_data in choices:
                        try:
                            ...
                            # Parse finish reason
                            if (
                                "finish_reason" in choice_data
                                and choice_data["finish_reason"] is not None
                            ):
                                try:
                                    parsed_finish_reason = FinishReason(
                                        choice_data["finish_reason"]
                                    )
                                    modified_choice_data["finish_reason"] = (
                                        parsed_finish_reason.value
                                    )
                                except Exception as e:
                                    self.logger.warning(
                                        f"Failed to parse streaming finish reason: {e}"
                                    )
                                    # Keep the raw finish reason if parsing fails
                                    modified_choice_data["finish_reason"] = choice_data[
                                        "finish_reason"
                                    ]

                            # Parse finish reason
                            if (
                                "delta" in choice_data
                                and choice_data["delta"] is not None
                            ):
                                try:
                                    parsed_delta = ChatCompletionStreamResponseDelta.model_validate(
                                        choice_data["delta"]
                                    )
                                    modified_choice_data["delta"] = parsed_delta
                                except Exception as e:
                                    self.logger.warning(
                                        f"Failed to parse streaming delta: {e}"
                                    )
                                    # Keep the raw delta if parsing fails
                                    modified_choice_data["delta"] = choice_data["delta"]

                            parsed_choice = (
                                ChatCompletionStreamResponseChoice.model_validate(
                                    modified_choice_data
                                )
                            )
                            unparsed_choice = parsed_choice.model_dump()

                            if "finish_reason" in unparsed_choice and isinstance(
                                unparsed_choice["finish_reason"], FinishReason
                            ):
                                parsed_choice["finish_reason"] = unparsed_choice[
                                    "finish_reason"
                                ].value

                            if "delta" in unparsed_choice and isinstance(
                                unparsed_choice["delta"],
                                ChatCompletionStreamResponseDelta,
                            ):
                                parsed_choice["delta"] = unparsed_choice[
                                    "delta"
                                ].model_dump()
                            ...
                            self.logger.warning(
                                f"Failed to parse streaming choice: {e}"
                            )
                ...
                if key == "tools" and isinstance(value, list):
                    ...
                    for tool_data in value:
                        try:
                            if isinstance(tool_data, ChatCompletionTool):
                                ...
                                parsed_tool = ChatCompletionTool.model_validate(
                                    tool_data
                                )
                    ...
                    for func_data in value:
                        try:
                            if isinstance(func_data, FunctionDefinition):
                                ...
                                parsed_function = FunctionDefinition.model_validate(
                                    func_data
                                )
                    ...
                    if isinstance(value, ReasoningConfig):
                        ...
                        try:
                            ...
                            request_data[key] = parsed_reasoning.model_dump(
                                exclude_none=True
                            )
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to parse reasoning config: {e}"
                            )
        ...
        for call_data in tool_calls_data:
            try:
                ...
                if "function" in call_data:
                    try:
                        parsed_function = ToolCallFunction.model_validate(
                            call_data["function"]
                        )
                ...
                parsed_tool_call = ChatCompletionToolCall.model_validate(
                    modified_call_data
                )
